...
public class MobFactory implements EnemyFactory {
    ...
    public AbstractEnemy createEnemy(double level) {
        locationX = (int) (Math.random() * (Config.screenWidth - ImageManager.MOB_ENEMY_IMAGE.getWidth()));
        locationY = (int) (Math.random() * Config.screenHeight * 0.2);
        int val = (int) (speedY * Math.sqrt(level));
        return new MobEnemy(locationX, locationY, speedX, Math.min(val, 15), (int) (hp * Math.sqrt(level)), score);
    }
}
